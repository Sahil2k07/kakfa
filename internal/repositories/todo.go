...
func (r *todoRepository) CreateTodo(todo *models.Todo) error {
	return connections.WDB.Create(todo).Error
}

func (r *todoRepository) UpdateTodo(id uint, todo *models.Todo) error {
	return connections.WDB.Model(&models.Todo{}).Where("id = ?", id).Updates(todo).Error
}

func (r *todoRepository) DeleteTodo(id uint, userID uint) error {
	res := connections.WDB.Where("id = ? AND user_id = ?", id, userID).Delete(&models.Todo{})
